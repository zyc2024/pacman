...
(** [dir_cmp m (x1,y1) (x2,y2) dir1 dir2] compares the distances between
    the tile at [(x2,y2)] and each of the adjacent tiles in directions
    [dir1] and [dir2] relative to [(x1,y1)] in maze [m]*)
...
(** [dir_rev_cmp m (x1, y1) (x2,y2)] is the reverse comparator (i.e.
    opposite of [dir_cmp]). *)
let dir_rev_cmp m (x1, y1) (x2, y2) dir1 dir2 =
  dir_cmp m (x1, y1) (x2, y2) dir2 dir1

(** [filter_dirs rev_dir m (x,y) dir_lst] is a subset of the list of
    directions [dir_lst] such that the tile in these direction(s)
    relative to position [(x,y)] is not occupied by a wall. None of the
    directions in the resulting set equals to direction [rev_dir]*)
let filter_dirs rev_dir m (x, y) dir_lst =
  List.filter
    (fun d ->
      d <> rev_dir
      && not (has_struct m Wall (neighbor_by_dir m (x, y) d)))
    dir_lst

(** [best_local_dir cmp get_pref g m target] is the preferred direction
    (sorted by cmp) based on function [get_pref] for a ghost [g] to move
    relative to its [target] on maze [m]. In the case that none of the
    preferred directions are valid, the remaining cardinal directions
    are sorted by [cmp].*)
...
let frightened_dir g m p =
  ...
  let dirs_away m g p =
    Maze.travel_dirs m g p |> Direction.complement
  in
...
(* [generic_move chase_algo scatter_point g m p] is a generic
   move-by-mode algorithm for ghosts. Considers the three modes of
   Ghosts: chase, frightened, scatter.*)
...
(** [pick_target g m desired] is [desired] when ghost [g] is not in the
    jail in maze [m]. Otherwise, return position of the jail exit in
    [m].*)
...
module BlinkyBehavior = struct
  let aggressive (g : Ghosts.t) (m : Maze.t) (p : Pacman.t) =
    let target =
      pick_target g m (get_tile_pos m (Pacman.get_coords p))
    in
...
module ClydeBehavior = struct
  let distancing (g : Ghosts.t) (m : Maze.t) (p : Pacman.t) =
    ...
    if Maze.distance m pcoords gcoords > float_of_int (8 * tile_size)
    then
      let dir = best_local_dir dir_cmp travel_dirs g m target in
      Ghosts.move m g dir ghost_speed
    else
  ...
  let move g m p =
    let scatter =
      pick_target g m (0, (Maze.height m - 1) * tile_size)
    in
...
module InkyBehavior = struct
  ...
  let assist blinky g m p =
    let desired =
      vector_target m (get_coords blinky)
        (get_tile_pos m (Pacman.get_coords p))
    in
