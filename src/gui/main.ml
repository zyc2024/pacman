...
(* this is the all-time high score recorded on the player's local
   computer, or 0 if one doesn't exist yet. *)
let highscore () =
  try
    let json =
      Yojson.Basic.from_file "highscore.json"
...
let update_highscore (game : GameState.t) =
  ...
  if score > highscore () then
    Yojson.to_file "highscore.json"
      (`Assoc [ ("high score", `Int score) ])
...
(** [spinwait time] makes the game do nothing but wait for [time]
    seconds. If the time argument is omitted, it waits indefinitely. *)
let spinwait ?(time = ~-.1.) () =
  let t = Sys.time () in
...
let draw_restart_note () =
  Graphics.moveto
    ((Graphics.size_x () / 2) - 75)
    ((Graphics.size_y () / 2) - 50);
  Graphics.draw_string "Press \"r\" to play again."

(** [prepare_graphics ()] clears the current graph and sets the
    properties for drawing strings*)
...
let draw_final_score game =
  ...
  draw_string
    ("FINAL SCORE: " ^ string_of_int (GameState.get_score game))

(** [next_state game] computes the next frame state after moving all
    ghosts, pacman, taking new user input, fruit check, etc*)
let next_state game =
...
(** [player_alive_check game] checks whether the player is alive in
    [game] and either modify the game for the next frame or reinitialize
    the game when the player is found dead. If the player is out of
    lives, the player is defeated.*)
...
and main score lives =
  try
    ...
    while true do
      ...
      else if GameState.paused !game then
        game := GameState.user_input !game
